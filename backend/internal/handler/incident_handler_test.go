...
func TestIncidentHandlerHappyPaths(t *testing.T) {
	...
	requests := []struct {
		...
		{http.MethodPost, "/incidents", `{"booking_id":"` + bookingID + `","type":"damage","description":"Product damaged"}`, http.StatusCreated},
		{http.MethodPost, "/items/" + targetID + "/reports", `{"type":"damage","description":"Product damaged"}`, http.StatusCreated},
		{http.MethodPost, "/customers/" + targetID + "/reports", `{"type":"other","description":"Bad behaviour"}`, http.StatusCreated},
